...
def compare_with_baselines(X_train, y_train, X_test, y_test):
    ...
    # 4. Complexity-Guided Ensemble (without optimizations)
    ...
    # 5. Complexity-Guided Ensemble (with Active Learning)
    print("\n5️⃣ PROPOSED: Complexity-Guided Ensemble (+ Active Learning)")
    print("-"*80)
    ensemble_al = ComplexityGuidedEnsemble(
        n_estimators=10,
    ...
    metrics, preds, _ = evaluate_classifier(
        ensemble_al, X_train, y_train, X_test, y_test,
        "CG-Ensemble (+ Active Learning)"
    )
    results['CG-Ensemble (+ AL)'] = metrics
    predictions['CG-Ensemble (+ AL)'] = preds
    
    # 6. Complexity-Guided Ensemble (Full - with Fitness)
    print("\n6️⃣ PROPOSED: Complexity-Guided Ensemble (Full)")
    print("-"*80)
    ...
    metrics, preds, _ = evaluate_classifier(
        ensemble_full, X_train, y_train, X_test, y_test,
        "CG-Ensemble (Full)"
    )
    results['CG-Ensemble (Full)'] = metrics
    predictions['CG-Ensemble (Full)'] = preds
...
def print_comparison_table(results):
    ...
    for metric in ['f1', 'auc', 'recall', 'precision']:
...
def analyze_ensemble_diversity(ensemble, X_train, y_train):
    ...
    # Fitness scores if available
    if ensemble.fitness_scores_ is not None:
        print(f"\n💪 Fitness Scores:")
        print(f"   Média: {ensemble.fitness_scores_.mean():.4f}")
        print(f"   Desvio: {ensemble.fitness_scores_.std():.4f}")
        print(f"   Min: {ensemble.fitness_scores_.min():.4f}")
        print(f"   Max: {ensemble.fitness_scores_.max():.4f}")
        
        for i, (mu, fitness) in enumerate(zip(stats['mu_values'], ensemble.fitness_scores_)):
...
def plot_results(results_df, save_path=None):
    ...
    ax3 = axes[1, 0]
    results_df[['precision', 'recall', 'f1']].plot(kind='bar', ax=ax3, rot=45)
...
def cross_validate_ensemble():
    ...
    methods = {
        ...
        'CG-Ensemble (+ AL)': ComplexityGuidedEnsemble(
            n_estimators=10,
            use_active_learning=True,
            use_fitness_optimization=False,
            verbose=0,
...
def main():
    ...
    print("  ✅ Aprendizado ativo para seleção de instâncias")
    print("  ✅ Otimização por fitness evolutivo")
